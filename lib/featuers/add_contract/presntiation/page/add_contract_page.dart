            ...
            BlocBuilder<AddContractBloc, AddContractState>(
              builder: (context, state) {
                if (state is LoadingState) {
                  ...
                  return LottieWidget(
                      text: state.message, lottie: AssetJson.error);
                } else if (state is AddContractError) {
                  return LottieWidget(
                      text: ' تعذر اضافة العقد', lottie: AssetJson.error);
                } else if (state is AddContractSuccess) {
                  return LottieWidget(
                      text: 'تم اضافة العقد بنجاح', lottie: AssetJson.success);
                } else {
                  return LottieWidget(
                      text: 'SomeThing Get Wrong', lottie: AssetJson.error);
                }
              },
            ),
          ],
